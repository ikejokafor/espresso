...
{
	m_address = allocate(MAX_AWP_PER_FAS * MAX_QUAD_PER_AWP * sizeof(cfg_t));
}
...
void AccelConfig::serialize()
{
	...
	for(int f = 0; f < NUM_FAS; f++)
	{
		for (int a = 0; a < MAX_AWP_PER_FAS; a++)
		{
			...
			{
				int idx 							= index2D(MAX_QUAD_PER_AWP, a, q);
				...
				cfg[idx].krnl1x1AddrArr 			= m_FAS_cfg_arr[f]->m_krnl1x1AddrArr;
				cfg[idx].partMapAddr				= m_FAS_cfg_arr[f]->m_partMapAddr;
				cfg[idx].resMapAddr					= m_FAS_cfg_arr[f]->m_resMapAddr;
				cfg[idx].biasAddr 					= m_FAS_cfg_arr[f]->m_biasAddr;
	...
	for(int f = 0; f < NUM_FAS; f++)
	{
		for (int a = 0; a < MAX_AWP_PER_FAS; a++)
		{
			...
			for (int q = 0; q < MAX_QUAD_PER_AWP; q++)
			{
				...
				m_FAS_cfg_arr[f]->m_krnl1x1AddrArr				= cfg[idx].krnl1x1AddrArr 		    ;
				m_FAS_cfg_arr[f]->m_partMapAddr				    = cfg[idx].partMapAddr			    ;
				m_FAS_cfg_arr[f]->m_resMapAddr				    = cfg[idx].resMapAddr				;
				m_FAS_cfg_arr[f]->m_biasAddr				    = cfg[idx].biasAddr 				;
