...
void dataTransform(vector<espresso::layerInfo_t<float>> &networkLayerInfo, vector<caffeDataParser::layerInfo_t> caffeDataParserLayerInfo) {
    ...
    for(uint32_t i = 0; i < caffeDataParserLayerInfo.size(); i++) {
        ...
        if(caffeDataParserLayerInfo[i].layerType == "Convolution" || caffeDataParserLayerInfo[i].layerType == "InnerProduct") {
            networkLayerInfo[i].filterData              = (float*)malloc    (   caffeDataParserLayerInfo[i].numKernelRows
            ...
            networkLayerInfo[i].biasData                = (float*)malloc(caffeDataParserLayerInfo[i].outputDepth * sizeof(float));
...
void dataTransform(vector<espresso::layerInfo_t<FixedPoint>> &networkLayerInfo, vector<caffeDataParser::layerInfo_t> caffeDataParserLayerInfo) {
    ...
    for(uint32_t i = 0; i < caffeDataParserLayerInfo.size(); i++) {
        ...
        if(caffeDataParserLayerInfo[i].layerType == "Convolution" || caffeDataParserLayerInfo[i].layerType == "InnerProduct") {
            ...
            for(int j = 0; j < caffeDataParserLayerInfo[i].outputDepth; j++) {
...
int main(int argc, char **argv) {
    ...
    if(layerIdx != -1) {
        ...
        if(network_fxPt->m_cnn[layerIdx]->m_blob.depth > 1 && network_fxPt->m_cnn[layerIdx]->m_blob.numRows > 1 && network_fxPt->m_cnn[layerIdx]->m_blob.numCols > 1) {
            for(int i = 0; i < network_fxPt->m_cnn[layerIdx]->m_blob.depth; i++) {
                for(int j = 0; j < network_fxPt->m_cnn[layerIdx]->m_blob.numRows; j++) {
                    for(int k = 0; k < network_fxPt->m_cnn[layerIdx]->m_blob.numCols; k++) {
                        fd << index3D(  
                                        network_fxPt->m_cnn[layerIdx]->m_blob.depth, 
                                        network_fxPt->m_cnn[layerIdx]->m_blob.numRows, 
                                        network_fxPt->m_cnn[layerIdx]->m_blob.numCols, 
                                        network_fxPt->m_cnn[layerIdx]->m_blob.data, i, j, k
                                     ).toFloat() << " ";
                    }
                    fd << endl;
                }
            ...
            for(int i = 0; i < network_fxPt->m_cnn[layerIdx]->m_blob.depth; i++) {
                fd << network_fxPt->m_cnn[layerIdx]->m_blob.data[i] << endl;
            }
        }
        fd.close();
    }
